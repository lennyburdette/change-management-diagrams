...
<div class="space-y-2 justify-center font-mono text-sm">
	...
	<TopologyContainer {lineConfigs} {boxConfigs}>
		<div class="relative grid topology gap-6 justify-center items-center">
			...
			<Component id="lb">Load<br />Balancer</Component>

			<div class="grid gap-2">
				<ServerPool id="gateway-blue" name="Gateway" cluster="Blue" />
				<ServerPool id="gateway-green" name="Gateway" cluster="Green" />
			</div>

			<div class="grid gap-2">
				<ServerPool id="subgraph-a" name="Subgraph A" />
				<ServerPool id="subgraph-b" name="Subgraph B" />
			</div>
